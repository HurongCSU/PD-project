...
def get_features_from_csv(pet5, pet6, pet16, pet17, pet11, pet21, t1, t2, clinical):
    ...
    for key in clinical_feature.keys():
        ...
        try:
            ...
            # progression binaried threshold
...
if __name__ == "__main__":
    ...
    k = 0
    # num-the number of selected feature
    for num in range(1, 5):

        models = [('GLM', LogisticRegression(random_state=seed)),
                  ('DT', DecisionTreeClassifier(random_state=seed)), ('BY', GaussianNB()),
                  ('SVM', SVC(probability=True, random_state=seed)),
                  ('BAG', BaggingClassifier(random_state=seed)), ('NNet', MLPClassifier(random_state=seed)),
                  ('BST', AdaBoostClassifier(random_state=seed)),
                  ('RF', RandomForestClassifier(random_state=seed))]

        sel = [
            ('RELF', SelectKBest(reliefF.reliefF, k=num)),
            ('TSCR', SelectKBest(t_score.t_score, k=num)),
            ('Fs', SelectKBest(k=num))
        ]
        ...
        for name, model in models:
            for kind, selection in sel:
                ...
                cv_results = model_selection.cross_val_score(pipe, train, y_train, scoring='roc_auc', cv=kfold)
                #print('cv', np.mean(cv_results))
                sheet.write(r, c, cv_results.mean())
                pipe.fit(train, y_train)

                Y_pred = pipe.predict_proba(val)
                fpr, tpr, thresholds = roc_curve(y_val.ravel(), Y_pred[:, -1].ravel())

                maxindex = (tpr - fpr).tolist().index(max(tpr - fpr))
                sheet_validate.write(r, c, roc_auc_score(y_val, Y_pred[:, -1]))
                threshold = thresholds[maxindex]
                # threshold = 0.36
                #print(threshold)
                #get_performance(Y_pred[:, -1], y_val, threshold)
                #print(Y_pred[:, -1], y_val)
                Y_pred = pipe.predict_proba(test)
                #get_performance(Y_pred[:, -1], y_test, threshold)
                sheet_test.write(r, c, roc_auc_score(y_test, Y_pred[:, -1]))
                #print(Y_pred[:, -1], y_test)
                r = r + 1
                print('ffffffffffffffffffffffffffffffffffffffffffff', r, c)
            c = c + 1
            r = k
        ...
        book.save('./new_csv/clinical' + str(num) + '.xls')
