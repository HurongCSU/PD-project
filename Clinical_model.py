...
def get_features_from_csv(pet5, pet6, pet16, pet17, pet11, pet21, t1, t2, clinical):
    ...
    pet21_feature = dict(zip(pp_t2f, pet21_dataset.iloc[:, 37:].values))
...
if __name__ == "__main__":
    ...
    print(len(y_train[y_train == 1]), len(y_val[y_val == 1]), len(y_test[y_test == 1]))
    ...
    sel = SelectKBest(reliefF.reliefF, k=num_fea)

    sel.fit(train, y_train)
    ...
    for i in sel.get_support():
        print(i)

    joblib.dump(sel, './model/clinical_features.pkl')
    joblib.dump(model, './model/clincial_model.pkl')
